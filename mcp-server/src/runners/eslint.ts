...
const ESLINT_CONFIG_PATH = resolve(
  __dirname,
  "../../node_modules/@lint-configs/eslint-config/eslint.config.js",
);
...
export async function runEslint(
  ...
  try {
    ...
    const { stdout: configCheck } = await execa("ls", ["-la", resolvedPath], {
      reject: false,
    });
    const hasOwnConfig =
      configCheck.includes("eslint.config") ||
      configCheck.includes(".eslintrc");
