...
export async function fix(input) {
    ...
    if (detectedLanguage === 'typescript' || detectedLanguage === 'auto') {
        ...
        const beforeCount = eslintBefore.reduce((sum, f) => sum + f.messages.filter((m) => m.fix !== undefined).length, 0);
        await runEslint(path, true);
        const eslintAfter = await runEslint(path, false);
        const afterCount = eslintAfter.reduce((sum, f) => sum + f.messages.filter((m) => m.fix !== undefined).length, 0);
