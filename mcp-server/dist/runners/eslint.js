...
const ESLINT_CONFIG_PATH = resolve(__dirname, '../../node_modules/@agentic-guardrails/eslint-config/eslint.config.js');
export async function runEslint(targetPath, fix) {
    ...
    try {
        const { stdout } = await execa(eslintBin, args, {
            ...
            cwd: resolvedPath.endsWith('.ts') || resolvedPath.endsWith('.js')
