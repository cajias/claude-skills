...
flag is single-source: passing `--ignore-path .gitignore` *overrides*
...
`apm compile -t claude` is a *translator*: it reads APM-native
...
dumps, raw shell sessions, commit-message snippets, or any "non-code"
block that isn't a real language: use ```` ```text ````. For real
languages, tag accurately (`yaml`, `json`, `python`, `bash`, `make`,
etc.).
...
markdownlint pass against the new SKILL.md *before* dropping it into
`plugins/<plugin>/skills/<name>/`. That saves a pre-commit round-trip.

**One-liner sanity check** (works without installing anything if `npx`
is on PATH):
...
`cc-slash-command-argument-hint-yaml`) blocked the first commit. The
husky-applied prettier pass ALSO touched the files (e.g. `*foo*` →
`_foo_`), so the diff coming out of the failed commit cycle was a mix
of legitimate markdownlint fixes plus prettier cosmetic noise — be
ready for the marketplace copy to drift slightly from the
`~/.claude/skills/` source even after a clean pre-commit.
