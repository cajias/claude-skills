...
class TestVectorMetadata:
    ...
    def test_to_dict_with_all_fields(self):
        ...
        assert (
            d["content_preview"] == "This hub connects all architecture-related notes"
        )
    ...
    def test_from_dict_with_empty_values(self):
        ...
        d = {"title": "Minimal Note", "obsidian_path": "fleeting/minimal.md"}
        meta = VectorMetadata.from_dict(d)
        assert meta.note_type == "fleeting"
...
PATCH_SESSION = "ai_zettelkasten.s3vectors._get_assumed_role_session"
...
class TestS3VectorsStore:
    ...
    def test_initialization_with_region(self):
        with patch(PATCH_SESSION) as mock_get_session:
            mock_get_session.return_value = _mock_session(MagicMock())
            S3VectorsStore("test-bucket", "test-index", region="eu-west-1")
            mock_get_session.assert_called_once_with("eu-west-1")
            ...
            "vectors": [
                {
                    "key": "note-1",
                    "distance": 0.1,
                    "metadata": {"title": "Note 1", "tags": "aws"},
                },
                {
                    "key": "note-2",
                    "distance": 0.3,
                    "metadata": {"title": "Note 2", "tags": "gcp"},
                },
            ]
        }
        mock_client = MagicMock()
    ...
    def test_query_with_filter(self):
        ...
        with patch(PATCH_SESSION) as mock_get_session:
            ...
            store = S3VectorsStore("test-bucket", "test-index")
            store.query([0.1] * 1536, top_k=5, filter={"status": "approved"})
            ...
            vectors = [
                (
                    ...
                    VectorMetadata(
                        ...
                        title="Note 1",
                        tags=["tag1"],
                        obsidian_path="fleeting/note1.md",
                    ),
                ),
                (
                    "key-2",
                    ...
                    VectorMetadata(
                        ...
                        title="Note 2",
                        tags=["tag2"],
                        obsidian_path="permanent/note2.md",
                    ),
                ),
            ]
...
class TestS3VectorsBulkOperations:
    ...
    def test_query_all_with_embeddings(self):
        ...
        with patch(PATCH_SESSION) as mock_get_session:
            ...
            store = S3VectorsStore("test-bucket", "test-index")
            store.query_all(filter={"status": "approved"}, include_embeddings=True)
