...
def process_skill_extraction(
    ...
    decision_event = create_base_event(
        ...
        data={
            ...
            'extraction_categories': list(set(
                infer_category_from_tags(s.get('tags', [])) for s in skills
            )),
            'skip_reason': None,
        }
    )
    ...
    for skill in skills:
        ...
        proposed_event = create_base_event(
            ...
            data={
                ...
                'problem_preview': (skill.get('problem', '')[:200]
                                   if skill.get('problem') else ''),
                'solution_preview': (skill.get('solution', '')[:200]
                                    if skill.get('solution') else ''),
            }
        )
...
def find_extraction_responses(messages: List[Dict[str, Any]]) -> List[str]:
    ...
    for msg in messages:
        ...
        for content in content_list:
            ...
            if 'CLAUDECEPTION' in text or '"skills"' in text:
                extraction_responses.append(text)
            elif any(phrase in text.lower() for phrase in REJECTION_PHRASES):
                extraction_responses.append(text)
    ...
    if not sys.stdin.isatty():
        try:
            ...
            if input_str:
                ...
                log(f"Received session data: {list(session_data.keys()) if isinstance(session_data, dict) else 'non-dict'}")
        except json.JSONDecodeError:
