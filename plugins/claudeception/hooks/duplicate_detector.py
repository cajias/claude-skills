...
# Configuration
SKILLS_DIR = Path(os.environ.get("CLAUDECEPTION_SKILLS_DIR", os.path.expanduser("~/.claude/my-claude-skills/skills")))
...
def calculate_multi_signal_similarity(skill_a: dict, skill_b: dict, vectorizer: TFIDFVectorizer) -> float:
    ...
    # Combined score
    combined = tfidf_sim * WEIGHT_TFIDF + tag_sim * WEIGHT_TAGS + trigger_sim * WEIGHT_TRIGGERS + name_sim * WEIGHT_NAME

    log(
        f"Similarity {skill_a.get('name')} vs {skill_b.get('name')}: "
...
def find_similar_skills(
    new_skill: dict, threshold: float = THRESHOLD_FLAG, skills_dir: Path = SKILLS_DIR
) -> list[dict]:
...
def should_reject_duplicate(
    new_skill: dict, threshold: float = THRESHOLD_REJECT, skills_dir: Path = SKILLS_DIR
) -> tuple[bool, str]:
    ...
    for match in similar:
        if match["similarity_score"] >= threshold:
            reason = (
                f"Too similar to existing skill '{match['skill_name']}' "
...
def main() -> None:
    ...
    parser.add_argument("action", choices=["check", "list", "test"], help="Action to perform")
    parser.add_argument("--name", help="Skill name to check")
    parser.add_argument("--description", help="Skill description")
    parser.add_argument("--tags", nargs="+", help="Skill tags")
    parser.add_argument("--threshold", type=float, default=THRESHOLD_FLAG, help="Similarity threshold")
    ...
    if args.action == "list":
        ...
        new_skill = {"name": args.name, "description": args.description or "", "tags": args.tags or []}
