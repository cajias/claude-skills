#!/usr/bin/env python3
"""Immutable scorer for the tabular-ring task. Part of the rsi-loop
...
def main():
    ...
    if not os.path.exists(os.path.join(task_dir, split_name, "instances.json")):
        print(f"no {split_name} instances for tabular-ring", file=sys.stderr)
        sys.exit(4)
    ...
    if per_instance is None:
        print(f"no usable {split_name} instances for tabular-ring", file=sys.stderr)
        sys.exit(4)
