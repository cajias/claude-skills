...
5. If the stress harness exists (`nodes/stress/stress_eval.py`), also run it
   on your fixed solution and report the real stress score it prints
   (report 0 if the harness is absent or the eval fails).
...
Report the real scorer outputs, even if your fix did not work.
