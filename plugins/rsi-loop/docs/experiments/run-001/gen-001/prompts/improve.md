...
1. Study the best node's code and both its scores, and the history of what
   has already been tried (including failed improvements — do not repeat
   them). If several past improvements tied the parent's public score, the
   public split is saturated: stop micro-tuning the same idea and target the
   stress signal with a change that helps a broader input class.
