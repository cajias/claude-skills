...
2. Pick ONE concrete improvement with a clear mechanism: a stronger
   ...
   distributions unlike the easy uniform case — clustered or multi-modal
   mixes are a classically hard class for one-pass rules). One idea per node.
   Special case worth checking FIRST when the best node comes from a
   recently discovered family that has never had an improve child: if its
   mechanism has an arbitrary structural parameter that was a first guess —
   number of restarts or candidate constructions in a portfolio, search
   width, pass count, subset-size cutoff — then widening that parameter as
   far as the time budget safely allows, or adding more genuinely different
   deterministic candidate rules to the portfolio, is usually the
   highest-value single change. Verify the runtime stays well within limits.
