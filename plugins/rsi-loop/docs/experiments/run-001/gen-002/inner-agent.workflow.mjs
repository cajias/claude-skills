export const meta = {
  name: 'aide2-family-diverse-inner-agent',
  description: 'AIDE2 tree-search inner agent: family-tagged parallel drafts, a seeded stress tie-breaker, and a search loop that spawns code-blind EXPLORE roots from banned-family lists whenever the public leaderboard ties or an improve stalls.',
  phases: [
    ...
    { title: 'Search', detail: 'debug buggy leaves; explore a NEW algorithm family on ties/stalls; otherwise improve the lexicographic best' },
  ],
}
...
// ── Phase 3: search — debug buggy leaves; explore a new family when the
// public leaderboard ties or an improve stalls; otherwise improve best ──
phase('Search')
let stalled = false
while (nodes.length < MAX_NODES) {
  ...
  if (buggyLeaves.length > 0) {
    ...
    // Public-tie detection: >=2 working nodes sharing the top public score
    // means the public metric has lost its gradient — inject a new family.
    const tiedAtTop = valid.filter((n) => n.public_score === best.public_score)
    op = stalled || tiedAtTop.length >= 2 ? 'explore' : 'improve'
    target = best
  }
  if (op === 'explore') {
