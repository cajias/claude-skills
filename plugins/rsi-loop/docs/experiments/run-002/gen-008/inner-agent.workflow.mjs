...
// gen-008 MULTI-DRAW SELECTION DE-NOISING (language only): a single paraphrase
// battery reliably discriminates on instruction-routing but names an unstable
// winner (its private generalization swings seed-to-seed) because ONE random
// paraphrase sample rewards a different tolerant parser each seed. Once a LANGUAGE
// battery has already discriminated (spread >= MIN_SPREAD), draw PROBE_DRAWS
// independent same-difficulty batteries, measure every candidate against each, and
// select on the MEAN robustness — averaging shrinks selection noise (~1/sqrt(draws))
// so the CONSISTENTLY robust parser wins instead of a per-draw lucky one. Only the
// language path (which discriminates) multi-draws; the tabular path keeps its
// single-draw escalate-on-saturation logic (its failure mode is saturation, not
// selection variance), and a saturated language battery never reaches this branch.
const PROBE_DRAWS = policy.probe_draws ?? 3
...
function batteryPrompt(round, draw = 0) {
  const escalation =
    round === 0
  ...
  // gen-008: independent-draw instruction. Downstream, several same-difficulty
  // language batteries are drawn and their per-candidate scores AVERAGED to
  // de-noise selection. Averaging only helps if the draws are INDEPENDENT, so an
  // extra draw must deliberately sample a DIFFERENT representative slice of the
  // public inputs and DIFFERENT synonym/rephrasing choices than a default draw
  // would — same difficulty, fresh sample. (draw 0 = the primary battery, no note.)
  const drawNote =
    draw > 0
      ? `\n\nINDEPENDENT DRAW #${draw}: this is one of several equally-hard batteries being drawn so their measurements can be AVERAGED. Pick a DIFFERENT representative subset of the public inputs and DIFFERENT (equally valid, equally hard) synonym/rephrasing choices than an obvious first pass would — do NOT reuse the same handful of inputs or the same canonical paraphrases every time. Same modality, same difficulty, genuinely fresh sample. Overwrite ${BATTERY_PATH}.`
      : ``
  return `${drawNote ? `${drawNote.trim()}\n\n` : ``}You are the ADVERSARIAL PROBE BUILDER for a research agent working on "${taskName}". You do NOT see or care about any candidate solution. Your only job is to synthesize a SHARED battery of HARD, equivalence-preserving perturbations of the public data, so a downstream evaluator can tell a brittle solution apart from one that truly generalizes.
...
let probe = { ran: false, spread: 0, escalations: 0, saturated: false, battery: null, results: [], draws: 0 }
...
async function runAdversarialProbe(candidates) {
  ...
  let battery = null
  let round = 0
  // ── Phase A: escalate battery difficulty until it DISCRIMINATES (or give up).
  // Unchanged from gen-006 — establishes that the instrument works for this
  // modality. For tabular this is the whole probe; for a saturated language
  // battery (e.g. bin-packing FFD) this ends saturated and we fall back.
  for (;;) {
  ...
  const saturated = spread < MIN_SPREAD
  const modality = battery && battery.modality ? battery.modality : 'language'

  // ── Phase B (gen-008): MULTI-DRAW SELECTION DE-NOISING — language only, and
  // only once Phase A confirmed the battery discriminates. A single paraphrase
  // draw names an unstable winner (its private generalization swings seed to
  // seed) because one random sample of rephrasings rewards a different tolerant
  // parser each seed. Draw PROBE_DRAWS independent SAME-difficulty batteries and
  // select on the MEAN per-node robustness so the CONSISTENTLY robust parser
  // wins. The Phase-A `saturated` verdict is PRESERVED (this refines the ranking,
  // it does not re-test whether the instrument works — averaging naturally
  // shrinks spread, so re-gating on it would spuriously trigger fallback).
  let draws = 1
  if (!saturated && modality === 'language' && PROBE_DRAWS > 1) {
    const sum = new Map() // node -> summed robustness across draws
    const cnt = new Map() // node -> number of draws it appeared in
    const fold = (rs) => {
      for (const r of rs) {
        if (!r || typeof r.node !== 'number') continue
        const v = clamp01(r.adversarial_robustness)
        sum.set(r.node, (sum.get(r.node) || 0) + v)
        cnt.set(r.node, (cnt.get(r.node) || 0) + 1)
      }
    }
    fold(results) // the discriminating Phase-A draw counts as draw #1
    while (draws < PROBE_DRAWS) {
      if (budget.total && budget.remaining() < PROBE_RESERVE / 3) {
        log(`probe: low budget, stopping multi-draw at ${draws}/${PROBE_DRAWS} draws`)
        break
      }
      const b2 = await agent(batteryPrompt(round, draws), {
        label: `probe-battery:draw-${draws}`,
        phase: 'Probe',
        schema: BATTERY_SCHEMA,
        model: MODEL,
        effort: EFFORT,
      })
      if (!b2 || !b2.battery_path) {
        log(`probe: draw-${draws} battery failed; stopping multi-draw`)
        break
      }
      const ev2 = await agent(probeEvalPrompt(candidates, b2), {
        label: `probe-eval:draw-${draws}`,
        phase: 'Probe',
        schema: PROBE_EVAL_SCHEMA,
        model: MODEL,
        effort: EFFORT,
      })
      fold(ev2 && Array.isArray(ev2.results) ? ev2.results : [])
      draws++
    }
    // Rebuild results as the per-node mean so the caller selects on the
    // de-noised ranking. Keep the Phase-A `saturated`/`spread` verdict.
    results = [...sum.keys()].map((node) => ({
      node,
      adversarial_robustness: sum.get(node) / cnt.get(node),
    }))
    const mvals = results.map((r) => clamp01(r.adversarial_robustness))
    const meanSpread = mvals.length ? Math.max(...mvals) - Math.min(...mvals) : 0
    log(
      `probe multi-draw (${draws} draw(s)): mean robustness [${results
        .map((r) => `n${r.node}=${clamp01(r.adversarial_robustness).toFixed(2)}`)
        .join(', ')}] mean-spread ${meanSpread.toFixed(3)}`
    )
  }
  return { spread, results, battery, escalations: round, saturated, draws }
}
...
if (pool.length <= 1) {
  ...
  probe = { ran: true, spread: r.spread, escalations: r.escalations, saturated: r.saturated, battery: r.battery, results: r.results, draws: r.draws }
