export const meta = {
  ...
  description: 'Tree-search inner agent: 5 parallel drafts, then a debug/improve loop selected on PUBLIC score (clean, non-degrading). A final ADVERSARIAL PROBE phase decouples the robustness instrument from the solver: one shared, hard, generalization-probing perturbation battery is generated ONCE from the public inputs and applied identically to every top-public candidate, so a brittle node and a generalizing node get different scores instead of both self-reporting 1.0. An anti-saturation guard escalates battery difficulty until the candidate scores spread; the returned best is the top-public candidate with the highest measured adversarial robustness.',
  phases: [
...
// PUBLIC_TIE_BAND of the max public score, capped at PROBE_TOPK. The probe only
// ever re-orders NEAR-TIED-public candidates, so it can never trade public score
// away (the gen-003 failure mode where combined-score steering dropped public).
...
const pool = basePool
  .filter((n) => n.public_score >= maxPublic - PUBLIC_TIE_BAND)
  .sort((a, b) => b.public_score - a.public_score || a.id - b.id)
  .slice(0, PROBE_TOPK)
