def pack(items: list[int], capacity: int) -> list[list[int]]:
    ...
    """
    # First Fit Decreasing (FFD): Sort items by size (descending), then pack greedily.
    ...
    # Initialize bins to track remaining capacity and items in each bin
    bins = []
    bin_loads = []
    ...
    for size, idx in indexed_items:
        ...
        for bin_id in range(len(bins)):
            if bin_loads[bin_id] + size <= capacity:
                bins[bin_id].append(idx)
                bin_loads[bin_id] += size
                placed = True
                break
